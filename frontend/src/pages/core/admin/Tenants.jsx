...
export default function Tenants() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      tenantApi.getAll(statusFilter === 'all' ? {} : { status: statusFilter })
  });
  ...
  const columns = [
    ...
    { field: 'subdomain', headerName: 'Subdomain', width: 130, valueGetter: (_, row) => row.subdomain || '-' },
    {
      field: 'status',
      headerName: 'Status',
      width: 110,
        ...
        <ActionColumnCell
          ...
          getActions={(row) => [
            ...
            },
            {
              ...
              variant: 'danger',
              onClick: async () => {
                const ok = await confirm('Delete this tenant? This action cannot be undone.', { confirmText: 'Delete', confirmVariant: 'danger' });
              ...
              ? [
                  {
                    ...
                    dividerBefore: true,
                    onClick: async () => {
                      const ok = await confirm('Suspend this tenant?', { confirmText: 'Suspend', confirmVariant: 'danger' });
      ...
      <PageContent>
        ...
        {themeTenant && (
          <TenantThemeModal
            ...
            tenant={themeTenant}
            onSubmit={(themeSetting) => updateThemeMutation.mutate({ id: themeTenant.id, themeSetting })}
...
const PREDEFINED_THEMES = [
  ...
  { primaryColor: '#4caf50', secondaryColor: '#ff9800', name: 'Green Orange' },
  ...
  { primaryColor: '#03a9f4', secondaryColor: '#9c27b0', name: 'Light Blue Purple' },
  { primaryColor: '#e91e63', secondaryColor: '#ff4081', name: 'Pink Magenta' },
  { primaryColor: '#ff6f00', secondaryColor: '#ffc107', name: 'Orange Amber' },
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Box sx={{ mb: 2 }}>
          ...
          <FormControlLabel
            ...
            sx={{ mt: 0.5, display: 'block', '& .MuiFormControlLabel-label': { fontSize: '0.875rem' } }}
          />
        </Box>
        <Box sx={{ mb: 2 }}>
          <Typography variant="caption" sx={{ fontSize: '0.75rem', color: 'text.secondary', fontWeight: 500, display: 'block', mb: 1 }}>
            Theme Colors
          </Typography>
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
            {PREDEFINED_THEMES.map((cfg, i) => (
              <Box
                ...
                onClick={() => { setPrimaryColor(cfg.primaryColor); setSecondaryColor(cfg.secondaryColor); }}
                title={cfg.name}
              >
                {isActive(cfg) && (
                  <CheckIcon sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', color: 'white', fontSize: 20, filter: 'drop-shadow(0 1px 2px rgba(0,0,0,0.5))' }} />
                )}
              </Box>
            ))}
          </Box>
        ...
        <div className="flex justify-end space-x-3 mt-6">
          ...
          <Button type="submit" disabled={isLoading}>{isLoading ? 'Saving...' : 'Save Theme'}</Button>
        </div>
      </form>
    </Modal>
  );
