...
export function FieldConfigDrawer({
  ...
  const supportsMultiple =
    formData.type === 'select' || formData.type === 'formReference' || formData.type === 'apiReference' || formData.type === 'file';

  const getAvailableFields = () => {
    if (!selectedFormSchema) return [];
    return (
      selectedFormSchema.sections?.flatMap((s) => s.fields) || selectedFormSchema.fields || []
    );
  };
          ...
          {formData.type === 'datepicker' && (
            <Box sx={{ mt: 2 }}>
              ...
              <RadioGroup
                ...
                onChange={(e) => handleChange('datePickerMode', e.target.value)}
                sx={{ mt: 1 }}
              >
              ...
              <TextField
                ...
                value={
                  formData.validation?.maxFileSize
                    ? (formData.validation.maxFileSize / (1024 * 1024)).toFixed(2)
                    : ''
                }
                onChange={(e) => {
