...
export const getModule = async (req, res) => {
  try {
    ...
    const [module] = await db.select().from(modules).where(eq(modules.id, parseInt(id))).limit(1);
...
export const createModule = async (req, res) => {
  try {
    ...
    const slugValue = slug || name.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');
    ...
    if (error.code === '23505') {
      return res.status(409).json({ error: 'Module slug already exists' });
    }
