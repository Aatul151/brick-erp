...
const FALLBACK_ITEMS = {
  form_studio: { name: 'Form Studio', path: '/form-studio', slug: 'form_studio', icon: 'ViewModule', roles: ['Site Admin', 'Client Admin', 'Client User'] },
  tenants: { name: 'Tenants', path: '/tenants', slug: 'tenants', icon: 'Business', roles: ['Site Admin'] },
  users: { name: 'Users', path: '/users', slug: 'users', icon: 'People', roles: ['Site Admin', 'Client Admin'] },
  roles: { name: 'Roles', path: '/roles', slug: 'roles', icon: 'AdminPanelSettings', roles: ['Site Admin'] },
  permissions: { name: 'Permissions', path: '/permissions', slug: 'permissions', icon: 'Security', roles: ['Site Admin'] },
  modules: { name: 'Modules', path: '/modules', slug: 'modules', icon: 'Extension', roles: ['Site Admin'] },
  settings: { name: 'Settings', path: '/settings', slug: 'settings', icon: 'Settings', roles: ['Site Admin', 'Client Admin'] },
  audit_logs: { name: 'Audit Logs', path: '/audit-logs', slug: 'audit_logs', icon: 'History', roles: ['Site Admin', 'Client Admin'] }
};

const ROOT_LEVEL_APPS = ['form_studio'];

const MENU_GROUPS = [
  { title: 'User Management', slugs: ['tenants', 'users', 'roles', 'permissions'], icon: GroupWorkIcon },
  { title: 'System Configuration', slugs: ['modules', 'settings'], icon: BuildIcon },
...
export function Sidebar({ open, onClose, collapsed }) {
    ...
    setGroupOpen((prev) => {
      ...
      MENU_GROUPS.forEach((g) => {
        const hasRouteInGroup = g.slugs.some((slug) => {
          ...
          return item && location === item.href;
        });
  ...
  const handleGroupClick = (groupTitle, e) => {
    if (collapsed) {
      ...
      setGroupOpen((prev) => ({ ...prev, [groupTitle]: !prev[groupTitle] }));
    }
  };
      ...
      >
        {!collapsed && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            >
              <AdminPanelSettingsIcon sx={{ fontSize: { xs: 26, sm: 24 } }} />
              ...
              >
                <ListItemIcon
                  sx={{
                    ...
                    '& svg': { fontSize: { xs: '1.35rem', sm: '1.3rem' } },
                  }}
                >
                  <DashboardIcon />
                </ListItemIcon>
                {!collapsed && (
                  <ListItemText
                    primary="Dashboard"
                    primaryTypographyProps={{ fontSize: { xs: '0.875rem', sm: '0.8125rem' }, fontWeight: isSelected('/dashboard') ? 500 : 400 }}
                  />
                )}
              </ListItemButton>
            </Tooltip>
                ...
                <Tooltip title={collapsed ? app.name : ''} placement="right" arrow>
                  ...
                  >
                    ...
                    {!collapsed && (
                      <ListItemText
                        primary={app.name}
                        primaryTypographyProps={{ fontSize: { xs: '0.875rem', sm: '0.8125rem' }, fontWeight: isSelected(app.href) ? 500 : 400 }}
                      />
                    )}
                  </ListItemButton>
                </Tooltip>
              ...
              return (
                <Box key={group.title}>
                  <ListItem disablePadding>
                    <Tooltip title={collapsed ? group.title : ''} placement="right" arrow>
                      <ListItemButton
                        ...
                        onClick={(e) => handleGroupClick(group.title, e)}
                        sx={itemStyles}
                      >
                        <ListItemIcon
                          sx={{
                        ...
                        {!collapsed && (
                          <>
                            <ListItemText
                              primary={group.title}
                              primaryTypographyProps={{ fontSize: { xs: '0.875rem', sm: '0.8125rem' }, fontWeight: isGroupSelected ? 500 : 400 }}
                            />
                  ...
                  <Popover
                    ...
                    PaperProps={{
                      sx: { mt: 0.5, minWidth: 200, boxShadow: '0 4px 12px rgba(0,0,0,0.15)', borderRadius: 2 },
                    }}
                  >
                    <MenuList dense>
                      {accessibleItems.map((item) => {
                        ...
                        return (
                          ...
                          >
                            <ListItemIcon sx={{ minWidth: 32, justifyContent: 'center', color: selected ? theme.palette.primary.main : 'text.secondary', '& svg': { fontSize: { xs: '1.2rem', sm: '1.15rem' } } }}>
                              {Icon && <Icon />}
                            </ListItemIcon>
                            <ListItemText primary={item.name} primaryTypographyProps={{ fontSize: { xs: '0.875rem', sm: '0.8125rem' }, fontWeight: selected ? 500 : 400 }} />
                          </MenuItem>
                        );
                      })}
                    </MenuList>
                          ...
                          <ListItem key={item.name} disablePadding>
                            <Tooltip title={collapsed ? item.name : ''} placement="right" arrow>
                              <ListItemButton
                                ...
                                sx={{ ...itemStyles, pl: 5, pr: 2.5 }}
                              >
                                <ListItemIcon sx={{ minWidth: 32, justifyContent: 'center', color: selected ? theme.palette.primary.main : 'text.secondary', '& svg': { fontSize: { xs: '1.2rem', sm: '1.15rem' } } }}>
                                  {Icon && <Icon />}
                                </ListItemIcon>
                                <ListItemText primary={item.name} primaryTypographyProps={{ fontSize: { xs: '0.8125rem', sm: '0.75rem' }, fontWeight: selected ? 500 : 400 }} />
                              </ListItemButton>
                            </Tooltip>
                          </ListItem>
                        );
      ...
      >
        {collapsed ? (
          <Tooltip title="Logout" placement="right" arrow>
            ...
            >
              <LogoutIcon sx={{ fontSize: { xs: '1.35rem', sm: '1.25rem' } }} />
            </ListItemButton>
          </Tooltip>
        ) : (
          <Button
