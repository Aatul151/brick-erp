...
export const loginSchema = z.object({
  ...
  if (!primary) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Email or mobile number is required', path: ['email'] });
    return;
  }
  if (z.string().email().safeParse(primary).success) return;
  const d = digitsOnly(primary);
  if (d.length >= 10 && d.length <= 15) return;
  ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Invalid email or mobile number', path: ['email'] });
});
