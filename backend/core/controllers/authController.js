  ...
  try {
    ...
    if (userRolesData.length > 0) {
      ...
      const perms = await db
        .select({ resourceName: permissions.resourceName, action: permissions.action })
    ...
    if (user[0].tenantId) {
      const [tenant] = await db.select({ name: tenants.name, themeSetting: tenants.themeSetting }).from(tenants).where(eq(tenants.id, user[0].tenantId)).limit(1);
