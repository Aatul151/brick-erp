...
export const authenticate = async (req, res, next) => {
    try {
        ...
        if (!loginUser.tenantId) {
            return res
                .status(403)
                .json({ error: "User is not associated with a tenant" });
        }
        ...
        if (!isSiteAdmin) {
            ...
            if (protectedTenant) {
                await logAudit({
                    ...
                    details: { reason: "Protected tenant access blocked" },
                    ipAddress: req.ip,
                });
                return res.status(403).json({
...
export const requireTenantAccess = (req, res, next) => {
    ...
    const requestedTenantId = parseTenantUuid(
