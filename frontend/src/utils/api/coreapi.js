...
export const formEntriesApi = {
  getAll: async ({ formName, page = 1, limit = 10, fields, filters }) => {
    const params = new URLSearchParams({ formName: String(formName), page: String(page), limit: String(limit) });
  ...
  update: async (entryId, payload) => {
    const body = await api.put(`/api/form-entries/${entryId}`, payload);
    return unwrapData(body);
  },
  delete: async (entryId, formName) => {
    await api.delete(`/api/form-entries/${entryId}?${new URLSearchParams({ formName })}`);
  }
};
...
export const fileUploadApi = {
  ...
  uploadFiles: async (formName, fieldName, files, recordId) => {
    ...
    const rid =
      recordId != null && String(recordId).trim() !== '' ? String(recordId).trim() : 'draft';
