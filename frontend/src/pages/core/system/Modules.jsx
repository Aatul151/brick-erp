...
export default function Modules() {
  ...
  const columns = [
    ...
    { field: 'slug', headerName: 'Slug', width: 140, valueGetter: (_, row) => row.slug || '-' },
    { field: 'icon', headerName: 'Icon', width: 130, valueGetter: (_, row) => row.icon || '-' },
        ...
        <ActionColumnCell
          ...
          getActions={(row) => [
            ...
            },
            {
              ...
              variant: 'danger',
              onClick: async () => {
                const ok = await confirm('Delete this module? Permissions using this module may be affected.', { confirmText: 'Delete', confirmVariant: 'danger' });
      ...
      <PageContent>
        ...
        {selectedModule && (
          <EditModuleModal
            ...
            onSubmit={(data) => updateMutation.mutate({ id: selectedModule.id, data })}
...
function CreateModuleModal({ isOpen, onClose, onSubmit, isLoading }) {
  ...
  const handleSubmit = async (e) => {
    ...
    await onSubmit({ ...formData, slug: slug || handleSlugFromName(formData.name) });
    setFormData({ name: '', slug: '', icon: '', description: '', sortOrder: 0, isActive: true });
  };
