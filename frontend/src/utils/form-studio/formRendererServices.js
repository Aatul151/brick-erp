...
export function createFormRendererServices(options = {}) {
    const recordId =
        options.recordId != null && String(options.recordId).trim() !== ""
            ? String(options.recordId).trim()
            : "draft";
    ...
    return {
        formReference: {
            fetchOptions: async (formName, fieldName) => {
                try {
                    ...
                    console.error(
                        `formReference.fetchOptions(${formName}):`,
                        e,
                    );
                    return [];
                }
        ...
        apiReference: {
            fetchOptions: (endpoint, labelField, valueField = "_id") =>
                apiReferenceService.fetchOptions(
                    endpoint,
                    labelField,
                    valueField,
                ),
        ...
        fileUpload: {
            uploadFiles: (formName, fieldName, files) =>
                fileUploadApi.uploadFiles(formName, fieldName, files, recordId),
        },
        fileBaseUrl:
