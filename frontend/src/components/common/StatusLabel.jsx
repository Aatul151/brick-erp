  ...
  const getConfig = () => {
    switch (variant) {
      case 'status': {
        ...
        if (v === 'active') return { className: 'bg-green-100 text-green-800', label: label ?? 'Active' };
        if (v === 'inactive') return { className: 'bg-yellow-100 text-yellow-800', label: label ?? 'Inactive' };
        return { className: 'bg-red-100 text-red-800', label: label ?? (value ? String(value).charAt(0).toUpperCase() + String(value).slice(1).toLowerCase() : 'Suspended') };
      }
      case 'scope': {
        const v = String(value || '').toLowerCase();
        if (v === 'global') return { className: 'bg-purple-100 text-purple-800', label: label ?? 'Global' };
        if (v === 'tenant') return { className: 'bg-blue-100 text-blue-800', label: label ?? 'Tenant' };
        return { className: 'bg-gray-100 text-gray-600', label: label ?? (value ? String(value).charAt(0).toUpperCase() + String(value).slice(1).toLowerCase() : '-') };
      }
      case 'active': {
      ...
      case 'action': {
        ...
        return { className: 'bg-blue-100 text-blue-800', label: label ?? v };
      }
      default:
        return { className: 'bg-gray-100 text-gray-600', label: label ?? (value ?? '-') };
    }
  };
