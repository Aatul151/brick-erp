    ...
    const mapped = rows.map((r) => {
      const submittedUser =
        r.submittedBy && r.submitterEmail
          ? { id: r.submittedBy, fullName: r.submitterName, email: r.submitterEmail }
          : null;
      return mapFormEntry(
        {
          id: r.id,
...
export const deleteFormEntry = async (req, res) => {
  try {
    ...
    const del = await db.delete(formEntries).where(and(...conditions)).returning({ id: formEntries.id });

    if (!del.length) return res.status(404).json({ error: 'Entry not found' });
