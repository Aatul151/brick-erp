...
const DEFAULT_MODULES = [
  ...
  { name: 'Permissions', slug: 'permissions', icon: 'Security', sortOrder: 4 },
  ...
  { name: 'Form Studio', slug: 'form_studio', icon: 'ViewModule', sortOrder: 8 }
];
  ...
  try {
    ...
    if (existingModules.length === 0) {
      ...
      if (formStudioModule.length === 0) {
        ...
        await db.insert(modules).values({ name: 'Form Studio', slug: 'form_studio', icon: 'ViewModule', sortOrder: 8 });
      }
    }
    ...
    const [siteAdminRole] = await db.insert(roles).values({
      name: 'Site Admin',
      description: 'Full system access across all tenants',
      scope: 'global'
    }).returning();

    const [clientAdminRole] = await db.insert(roles).values({
      name: 'Client Admin',
      description: 'Full access within tenant organization',
      scope: 'tenant'
    }).returning();

    const [clientUserRole] = await db.insert(roles).values({
      name: 'Client User',
