...
export default function Users() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      userApi.getAll(statusFilter === 'all' ? {} : { status: statusFilter })
  });
        ...
        <ActionColumnCell
          ...
          getActions={(row) => [
            ...
            },
            {
              ...
              variant: 'danger',
              onClick: async () => {
                const ok = await confirm('Delete this user? This action cannot be undone.', { confirmText: 'Delete', confirmVariant: 'danger' });
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Create New User">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {roles.filter(r => isSiteAdmin || r.scope === 'tenant').map(role => (
...
function InviteUserModal({ isOpen, onClose, onSubmit, isLoading, roles }) {
  const [formData, setFormData] = useState({ email: '', fullName: '', roleIds: [] });
  const [error, setError] = useState('');
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Invite User">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {roles.filter(r => r.scope === 'tenant').map(role => (
            <label key={role.id} className="flex items-center mb-2">
              <input
                type="checkbox"
                checked={formData.roleIds.includes(role.id)}
